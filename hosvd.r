...
unfolding2 <- matrix(aperm(tenseur, perm=c(2, 3, 1)), nrow=7) #e, g * t_max + t

unfolding3 <- matrix(aperm(tenseur, perm=c(3, 1, 2)), nrow=9000) #t, e * g_max + g
...
core.tensor <- apply(tenseur, MARGIN=c(2, 3), FUN=prod.mat, matrice=t(Conj(decomp1$u)))
core.tensor <- apply(core.tensor, MARGIN=c(1, 3), FUN=prod.mat, matrice=t(Conj(decomp2$u)))
core.tensor <- apply(core.tensor, MARGIN=c(1, 2), FUN=prod.mat, matrice=t(Conj(decomp3$u)))

sortie <- sort(Mod(core.tensor), decreasing=TRUE)

write.table(sortie, "code/resultatHOSVD.txt", row.names=FALSE, col.names=FALSE)
